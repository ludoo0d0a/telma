<template>
  ...
  <div
    v-else-if="!locationInfo.station"
    class="card dashboard-card current-location-widget"
    @click="handleClick"
  >
    <div class="card-content">
  ...
  <div
    v-else
    class="card dashboard-card current-location-widget has-background-info"
    @click="handleClick"
  >
    <div class="card-content">
      <div class="media">
        <div class="media-left">
          <span class="icon is-large has-text-white">
            <Train v-if="locationInfo.train" :size="32" />
            <MapPin v-else :size="32" />
          </span>
        </div>
        <div class="media-content">
          <template v-if="locationInfo.train">
...
const DEFAULT_RADIUS_NEARBY = 5000;
const DEFAULT_RADIUS_NEARBY_LARGE = 10000;
...
const hasDetectedRef = ref(false);
...
const findTrainAtLocation = async (
  ...
  try {
    ...
    for (const train of allTrains.slice(0, 3)) {
      const vehicleJourneyLink = train.links?.find(
        (link) => link.type === 'vehicle_journey' || link.id?.includes('vehicle_journey')
      );
      ...
      try {
        ...
        for (const stopTime of stopTimes) {
          ...
          const arrivalTime =
            stopTime.arrival_date_time ||
            stopTime.base_arrival_date_time ||
            stopTime.utc_arrival_time;
          const departureTime =
            stopTime.departure_date_time ||
            stopTime.base_departure_date_time ||
            stopTime.utc_departure_time;
          ...
          if (arrivalTime || departureTime) {
            ...
            if (distance < 100 && timeDiff < 5 * 60 * 1000) {
              const displayInfo = train.display_informations || {
                ...
                network:
                  (vehicleJourney.journey_pattern as any)?.route?.line?.network?.name || 'SNCF',
...
const detectCurrentLocation = async () => {
  if (hasDetectedRef.value) {
    return;
  }
  hasDetectedRef.value = true;
